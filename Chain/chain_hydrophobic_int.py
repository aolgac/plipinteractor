...
hydrophobic_bond.drop(columns=["RESNR","RESTYPE","RESNR_LIG","RESTYPE_LIG","LIGCARBONIDX","PROTCARBONIDX"],inplace=True)
# count occurences by each source & add as a new column; group by chain & reset index by count_by_source
hydrophobic_bond["COUNT_by_source"] = hydrophobic_bond.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).transform("size")
hydrophobic_b=hydrophobic_bond.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).size().reset_index(name="COUNT_by_source")
# count overall occurences
hydrophobic_b["COUNT"] = hydrophobic_b.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE"]).transform("size")
...
hyd_b =hydrophobic_b.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","COUNT"])["SOURCES"].apply(lambda x: "|".join(x)).reset_index()
