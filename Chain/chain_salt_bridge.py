...
salt_bridge.drop(columns=["RESNR","RESTYPE","RESNR_LIG","RESTYPE_LIG","PROT_IDX_LIST","LIG_IDX_LIST"],inplace=True)
# count occurences by each source & add as a new column; group by chain & reset index by count_by_source
salt_bridge["COUNT_by_source"] = salt_bridge.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).transform("size")
salt_b=salt_bridge.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).size().reset_index(name="COUNT_by_source")
# count overall occurences
salt_b["COUNT"] = salt_b.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE"]).transform("size")
...
s_b = salt_b.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","COUNT"])["SOURCES"].apply(lambda x: "|".join(x)).reset_index()
