...
pi_cation.drop(columns=["RESNR","RESTYPE","RESNR_LIG","RESTYPE_LIG","PROT_IDX_LIST","LIG_IDX_LIST"],inplace=True)
# count occurences by each source & add as a new column; group by chain & reset index by count_by_source
pi_cation["COUNT_by_source"] = pi_cation.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).transform("size")
pi_c=pi_cation.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).size().reset_index(name="COUNT_by_source")
# count overall occurences
pi_c["COUNT"] = pi_c.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE"]).transform("size")
...
p_c = pi_c.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","COUNT"])["SOURCES"].apply(lambda x: "|".join(x)).reset_index()
