...
hydrogen_bond.drop(columns=["RESNR","RESTYPE","RESNR_LIG","RESTYPE_LIG","DONORIDX","ACCEPTORIDX"],inplace=True)
# count occurences by each source & add as a new column; group by chain & reset index by count_by_source
hydrogen_bond["COUNT_by_source"] = hydrogen_bond.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).transform("size")
hydrogen_b=hydrogen_bond.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","SOURCE"]).size().reset_index(name="COUNT_by_source")
# count overall occurences
hydrogen_b["COUNT"] = hydrogen_b.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE"]).transform("size")
...
h_b = hydrogen_b.groupby(["RESCHAIN","RESCHAIN_LIG","INTERACTION TYPE","COUNT"])["SOURCES"].apply(lambda x: "|".join(x)).reset_index()
